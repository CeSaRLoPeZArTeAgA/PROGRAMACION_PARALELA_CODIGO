...
max_iter = 500# maximo cantidad de generaciones
...
def representativo(archivo):
    ...
    if archivo_encontrado is None:
        raise FileNotFoundError(f"❌ No se encontró '{archivo}' en la carpeta:\n{os.getcwd()}\n"
                                f"📁 Archivos presentes: {archivos}")
...
def representativo(archivo):
    ...
    m,n = im.size# 256x256
    imarray = np.array(im,np.float32)# convertir de imagen a numpy
    patron = imarray[1:m-1,1:n-1]# 254x254
    EE = orientacion(patron,14)# 18x18
    return np.asarray(EE).reshape(-1)# 1x324
'''
...
# Inicia el proceso de entrenamiento de la RN
print("---------------------")
...
print(M_index)
print("---------------------")
print("Realizar el mapeo o comprobación: ")
mapear('arco3.tif',W,M_index)
print(EEE)
print(EEE.shape)
