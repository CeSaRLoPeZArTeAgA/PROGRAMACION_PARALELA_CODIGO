...
def inicializar_esferas():
    ...
    # Obtener numero de procesadores y usarlo como nnmero de esferas
    num_procesadores = multiprocessing.cpu_count()

     # Todas las esferas saldrán desde el mismo punto (0,0,0)
    posiciones_x = [0 for _ in range(num_procesadores)]
    
    # Generar 8 colores distintos
    colores = generar_colores_vivos(num_procesadores)

    # crear esferas aleatorias (ángulo en [20,80], velocidad en [20,40])
    initial_params = []
    esferas_local = []
    for i, px in enumerate(posiciones_x):
        ang = random.randint(20, 80)
        vel = random.randint(20, 40)
        color = colores[i % len(colores)]
        e = Esfera(color, ang, vel)
        e.posicion = [0, 0, 0]
        esferas_local.append(e)
        initial_params.append((ang, vel))

    esferas = esferas_local
...
def draw_axes(length=100.0):
    ...
    glVertex3f(3.4*length, 0.15, 0.0)
...
def display_info():
    ...
    info = ["MOVIMIENTO DE ESFERAS USANDO PARALELISMO ", ""]
...
def run_threaded_simulation_visible(params_list):
    ...
    # decidir posiciones X de las esferas basado en la cantidad de params
    n = len(params_list)
    span = 60
    if n == 1:
        posiciones_x = [0]
    else:
        posiciones_x = [(-span + i * (2 * span) / (n - 1)) for i in range(n)]
    ...
    for i, ((ang, vel), posx) in enumerate(zip(params_list, posiciones_x)):
        color = colores[i % len(colores)]
        e = Esfera(color, ang, vel)
        e.posicion = [posx, 0, 0]
        esferas.append(e)
...
def main():
    ...
    glutCreateWindow(b"Practica 3 Paralela:  Movimiento Parabolico - N Esferas 3D")
    try:
        glutShowWindow()
    except:
        pass
